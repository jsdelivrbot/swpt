var r=angular.module('re',["tm.pagination"]);
...
r.controller('rectr',["$scope","getData",function($scope,getData){
    ...
    $scope.courseware_type = getData.courseware;
   ...
   //初始化变量
    ...
    $scope.cur_teaching={
        ...
        "subject_key":null, // 不知道
        "subject_name":null, // 科目名称
        "school_period_id":$scope.school_period[0].id,  // 校段id
        "school_period_name":$scope.school_period[0].name,  // 校段名称
        "version_id":null,  // 版本id
        "version_key":null, // 不知道
        "version_name":null, // 版本名称
        "semester_id":null, // 册数id
        "semester_key":null, // 不知道
        "semester_name":null,  // 册数名称
        "library_type_id":$scope.library_type[3].id,  // 选择 更多资源 - targetId
        "library_type_title":$scope.library_type[3].title,  // 选择 更多资源 - targetName
        "teaching_plan_type_id":$scope.teaching_plan_type[2].id,  // 切换教材 - targetId
        "teaching_plan_type_title":$scope.teaching_plan_type[2].title, // 切换教材 - targetName
        "courseware_type_id":$scope.courseware_type[0].id,  //选择文件类型 ID
        "courseware_type_title":$scope.courseware_type[0].title, //选择文件类型 名称
        "section_id":null,
        "section_key":null,
        "section_name":null,
        "r_section_id":null,
    ...
    $scope.version=null;
    $scope.cur_teaching.teaching_plan_type_title = "课件";//页面打开默认显示
    $scope.cur_teaching.school_period_name = "小学";//页面打开默认显示
    $scope.cur_teaching.subject_name  ="语文";//页面打开默认显示
    $scope.cur_teaching.semester_name ="一年级上册";//页面打开默认显示
    $scope.cur_teaching.courseware_type_title = "文档";//页面打开默认显示
    //$scope.semester={};
    $scope.semester=null;
    $scope.showlist=function(){
        $scope.listContentType='list';
        var listsname = "ls"+$scope.cur_teaching.library_type_id+"_"+$scope.cur_teaching.subject_id
						+"_"+$scope.cur_teaching.version_id+"_"+$scope.cur_teaching.semester_id+"_"
						+$scope.cur_teaching.section_id+"_"+$scope.cur_teaching.r_section_id;
        var node='';
		//获得条件ID，比如课程、册数、学段等
        if($scope.cur_teaching.r_section_id!=null){
            ...
            node=$scope.cur_teaching.section_key;
		...
		//更多资源ID=2/参考资源ID=5
        if($scope.cur_teaching.library_type_id==5){
            $scope.listContentType='partlist';
			url = "/repository/getRepositoryInfoPaging?start="+$scope.paginationConf.currentPage
			 +"&limit="+$scope.paginationConf.itemsPerPage+"&repositoryType="+$scope.cur_teaching.teaching_plan_type_title
			 +"&repositoryVersion="+$scope.cur_teaching.version_name
			 +"&studySection="+$scope.cur_teaching.school_period_name
			 +"&subjectName="+$scope.cur_teaching.subject_name
			 +"&className="+$scope.cur_teaching.semester_name
			 +"&fileTypeName="+$scope.cur_teaching.courseware_type_title;
        }else{
        ...
        if(node!=''){
			//正确请求成功时处理
			getData.getUrlData(url,$scope.listContentType).then(function (d) {
                if($scope.cur_teaching.library_type_id==5){
	...
	　　　　　　perPageOptions: [10, 20, 30, 40, 50],//可选择的每页展示多少条数据
	           onChange:function(){
	           	getData.getUrlData("/repository/getRepositoryInfoPaging?start="
	           	+$scope.paginationConf.currentPage+"&limit="
	           	+$scope.paginationConf.itemsPerPage+"&repositoryType="
	           	+$scope.cur_teaching.teaching_plan_type_title+
	           	"&repositoryVersion="+$scope.cur_teaching.version_name+
	           	"&studySection="+$scope.cur_teaching.school_period_name
	           	+"&subjectName="+$scope.cur_teaching.subject_name
	           	+"&className="+$scope.cur_teaching.semester_name+
	           	"&fileTypeName="+$scope.cur_teaching.courseware_type_title).then(function(d){
	           		    $scope.redata = d.data.msg;  
	           		    $scope.showlist();
	           	})
	        }  
　　　};
	 ...
	 //默认获取总页数,同步数据
	 $scope.cur_teaching.version_name ="人教版";//默认打开页面显示
      	$.ajax({
		    url:courseware_root_url+"/repository/getRepositoryInfoCount?repositoryType="
		        +$scope.cur_teaching.teaching_plan_type_title+
		       "&repositoryVersion="+$scope.cur_teaching.version_name+
		        "&studySection="+$scope.cur_teaching.school_period_name+
		        "&subjectName="+$scope.cur_teaching.subject_name+
		    ...
		    success: function(res){
		    	$scope.paginationConf.totalItems = res.msg;  //获取分页总页数
		        $scope.page();
		    }
		  });
      $scope.btmp=null;
    ...
    $scope.sysLibraryConfirm=function(){
		...
		if ( $scope.tmp.library_type_id== 5 ){
			$scope.tmp.teaching_plan_type_id=$scope.teaching_plan_type[5].id;
			$scope.tmp.teaching_plan_type_title=$scope.teaching_plan_type[5].title;
		}
    ...
    $scope.getSection=function(id,key,title){
        ...
        $scope.tmp.courseware_type_id = id;
        $scope.tmp.courseware_type_title = title; 
        $scope.tmp.r_section_id=null;
    ...
    $scope.chapterSubmit=function(){
    	$scope.paginationConf.currentPage = 1;//默认显示第一页
        $scope.cur_teaching=angular.copy($scope.tmp);
         //获取总页数 同步数据
      	  $.ajax({
		    ...
		    dataType:"JSON",
		    success: function(res){
		    	$scope.paginationConf.totalItems = res.msg;//获取总页数
		    	  $scope.showlist(); 
		    }
		   }); 
    ...
    $scope.reDownload = function(){
    	 var android =  navigator.userAgent.toLowerCase();
                 ...
                 if(bool){
                        window.loginMessage.loaderfile()
            }
    }
}])
...
.controller('reUpfileDesctr',["$scope","$routeParams","$compile","getData",function($scope,$routeParams,$compile,getData){
    ...
    .controller('reDetailedCtr',["$scope","$routeParams","getData",function($scope,$routeParams,getData, $sce){
		...
		getData.getUrlData("/repository/getRepositoryInfoById?id="+fid).then(function(d){
            $scope.des=d.data.msg;
            if($scope.des.repositorySuffix == "swf"){
            	$("#video").addClass("activeDes");
            	$("#flash").removeClass("activeDes");
            }else{
            	$("#flash").addClass("activeDes");
            	$("#video").removeClass("activeDes");
            }
         
       })
		//本地下载
        /*$scope.reDownload=function(downUrl){
            getData.getUrlData(url).then(function(d){
                var urlpath=d.data.msg;
                console.log(urlpath);
   ...
   .controller('reSearchCtr',["$scope","getData",function($scope,getData){
   	  $scope.title = null;//文本框输入的内容
   	  $scope.paginationConf = {
   	  	currentPage :1,//默认选中第一页
   	  	itemsPerPage :10,//默认显示10条数据
   	  	totalItems : null //默认总数为空
   	  }
   	
   	   $scope.search = function(){
   	   	$scope.paginationConf.currentPage = 1;//默认每次搜索为当前页为第一页
	   	   	if($scope.title==null||$scope.title.trim('')==''){
	                alert('没有输入搜索内容');
	                return false;
	        }
	   	  getData.getUrlData("/repository/getLikeRepositoryInfo?start="
				           	+$scope.paginationConf.currentPage
				           	+"&limit="+$scope.paginationConf.itemsPerPage
				           	+"&context="+$scope.title,'list').then(function(d){
                	          $scope.searchData = d.data.msg;
	   	    })    	
	   	   	getData.getUrlData("/repository/getRepositoryLikeInfoCount?context="+$scope.title,'list').then(function(d){
                	$scope.paginationConf.totalItems = d.data.msg;
	   	    })
...
$scope.paginationConf.currentPage+"&limit="+$scope.paginationConf.itemsPerPage
