...
//初始化区域图的对象
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		if (e.data.inthis.$nowType == "start") {

            e.data.inthis.addNode("start_"+e.data.inthis.$max, { name: "start_"+e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, command: "" , TriggerName: "" , TriggerType: "", TableName: "", FunctionName: ""});
            e.data.inthis.$max++;

        } 
        else if (e.data.inthis.$nowType == "chat") {
        e.data.inthis.addNode("chat_" + e.data.inthis.$max, { name: "chat_" + e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, command: "", sql: "",chat_table:"",chat_tbcolumn:"",chat_logic:"",chat_group:"",chat_where:"" });
            e.data.inthis.$max++;
        }
        else if (e.data.inthis.$nowType == "complex") {
                e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, { name: "node_" + e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, IsParent: "True", command: "", TatbleName: "", TatblField: "", Rule: "", Result: "" });
	            var node_A = e.data.inthis.$id + "_node_" + e.data.inthis.$max;
	            e.data.inthis.$max++;

	            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, { name: "node_complex_Yes_" + e.data.inthis.$max, left: X - 60, top: Y + 60, type: "chat", IsParent: "False", command: "", sql: "" });
	            var node_B = e.data.inthis.$id + "_node_" + e.data.inthis.$max;
	            e.data.inthis.$max++;

	            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, { name: "node_complex_No_" + e.data.inthis.$max, left: X + 60, top: Y + 60, type: "chat", IsParent: "False", command: "", sql: "" });
	            var node_C = e.data.inthis.$id + "_node_" + e.data.inthis.$max;
	            e.data.inthis.$max++;

	            e.data.inthis.addLine(e.data.inthis.$id + "_line_" + e.data.inthis.$max, { from: node_A, to: node_B, name: "是", command: "" });
	            e.data.inthis.$max++;

	            e.data.inthis.addLine(e.data.inthis.$id + "_line_" + e.data.inthis.$max, { from: node_A, to: node_C, name: "否", command: "" });
	            e.data.inthis.$max++;
	        }else if(e.data.inthis.$nowType =="role"){
	        e.data.inthis.addNode("role_" + e.data.inthis.$max, { name: "role_" + e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, command: "", root: "", isfine: "" });
	            e.data.inthis.$max++;
	        }else {
            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, { name: "node_" + e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, command: "" });
            e.data.inthis.$max++;
        }
...
GooFlow.prototype = {
    ...
    addNode: function(id, json) {
        ...
        if (json.type.indexOf(" round") < 0) {
            ...
            if (json.type == "complex") {
                ...
                this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_IfElse'></div><div class='rs_close'></div></div></div>");
    ...
    initWorkForNode: function() {
        ...
        //		this.$workArea.delegate(".ico + td", "click", { inthis: this }, function (e) {
        //		e.data.inthis.GetNodesJson(); 
        ...
        // 置换成 IfElse
        this.$workArea.delegate(".rs_IfElse", "click", { inthis: this }, function(e) {
            if (!e) e = window.event;
            var node_A = e.data.inthis.$focus;

            if (e.data.inthis.$nodeData[node_A].IsParent == "True") {
                return;
            }

            e.data.inthis.$nodeData[node_A].IsParent = "True";

            var X = e.data.inthis.$nodeData[node_A].left;

            var Y = e.data.inthis.$nodeData[node_A].top;

            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, { name: "node_" + e.data.inthis.$max, left: X - 60, top: Y + 60, type: "chat", IsParent: "False", command: "DFDFGDFG4" });
            var node_B = e.data.inthis.$id + "_node_" + e.data.inthis.$max;
            e.data.inthis.$max++;

            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, { name: "node_" + e.data.inthis.$max, left: X + 60, top: Y + 60, type: "chat", IsParent: "False", command: "4356789" });
            var node_C = e.data.inthis.$id + "_node_" + e.data.inthis.$max;
            e.data.inthis.$max++;
    ...
    SetNodesJson: function(title, TriggerName, TriggerType, TableName, FunctionName) {

        this.setName(this.$focus, title, "node");
        this.$nodeData[this.$focus].TriggerName = TriggerName;
        this.$nodeData[this.$focus].TriggerType = TriggerType;
        this.$nodeData[this.$focus].TableName = TableName;
        this.$nodeData[this.$focus].FunctionName = FunctionName;
        if (this.$nodeData[this.$focus].name == title) {
            alert("存储成功");
        } else {
            alert("存储失败")
        }
    ...
    SetRootJson: function(root, isfine) {
        this.$nodeData[this.$focus].root = root;
        this.$nodeData[this.$focus].isfine = isfine;
    },
    SetCharJson: function(name, sql, tables, sql_Column, chat_where, chat_sort, chat_grouping) {
        this.setName(this.$focus, name, "node");
        this.$nodeData[this.$focus].chat_table = tables;
        this.$nodeData[this.$focus].chat_tbcolumn = sql_Column;
        this.$nodeData[this.$focus].chat_where = chat_where;
        this.$nodeData[this.$focus].chat_logic = chat_sort;
        this.$nodeData[this.$focus].chat_group = chat_grouping;
        this.$nodeData[this.$focus].sql = sql;
        if (this.$nodeData[this.$focus].name == name) {
            alert("存储成功");
        } else {
            alert("存储失败")
        }
    ...
    SetComm: function(com) {
        this.$nodeData[this.$focus].command = com;
        if (this.$nodeData[this.$focus].command == com) {
            alert("存储成功");
        } else {
            alert("存储失败")
        }
    ...
    Setcomplex: function(TatbleName, TatblField, Rule, Result) {

        this.$nodeData[this.$focus].TatbleName = TatbleName;
        this.$nodeData[this.$focus].TatblField = TatblField;
        this.$nodeData[this.$focus].Rule = Rule;
        this.$nodeData[this.$focus].Result = Result;

        if (this.$nodeData[this.$focus].Result == Result) {
            alert("存储成功");
        } else {
            alert("存储失败")
        }
    ...
    addLine: function(id, json) {
        ...
        //避免两个节点间不能有一条以上同向接连线
        for (var k in this.$lineData) {
            if ((json.from == this.$lineData[k].from && json.from =="start_1")) return;
