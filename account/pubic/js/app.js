...
app.config(['$stateProvider','$urlRouterProvider',function($stateProvider,$urlRouterProvider){
    ...
    $stateProvider
        ...
        .state('addr',{
            url:'/addr',
            templateUrl:'./pubic/tpls/addr.html'
        })
        .state('user',{
          url:'/user',
...
app.controller("myController",["$scope","$http",function ($scope,$http) {
    ...
    $scope.checkDevice();
    $scope.browser.versions[ 'android' ] || $scope.browser.versions[ 'mobile' ] ? $scope.isPhone = true : $scope.isPhone = false;
    //判断是否有来源
    ...
    if (!!$scope.userInfo){
        ...
        $http({
            url:'http://1388w.cn:20896/htUserService/getUserById?id='+$scope.userInfo[ 'id' ]
        }).then(function (res) {
    ...
    $scope.chSave =function () {
        ...
        $http({
            method:'POST',
            url:'http://1388w.cn:20896/htUserService/UpdateUserAndPic',
            data: {
