...
//------------------ 样式定义 ---------------------------//
//功能按钮同样样式
var s_tiannet_turn_base = "height:16px;font-size:9pt;color:white;border:0 solid #CCCCCC;cursor:pointer;background-color:#2650A6;";
//翻年、月等的按钮
...
//字体样式
var s_tiannet_font = "color:#FFCC00;font-size:9pt;cursor:pointer;";
//链接的样式
...
function setDay(event,obj) {
    ...
    tiannetPopCalendar(event);
    document.getElementById("divTiannetDate").style.zIndex = 9999;
}
//用户主调函数－选择日期和小时
function setDayH(event, obj) {
    tiannetOutObject = obj;
    m_bolShowHour = true;
...
for (var i = 0; i < 5; i++) {
    ...
    'onClick="tiannetDay=this.innerText||this.textContent;tiannetSetValue(true);" '
...
document.write('<td align="center" id="tdTiannetDay35" onClick="tiannetDay=this.innerText||this.textContent;tiannetSetValue(true);" '
    + ' style="' + s_tiannet_day + '">&nbsp;</td>');
document.write('<td align="center" id="tdTiannetDay36" onClick="tiannetDay=this.innerText||this.textContent;tiannetSetValue(true);" '
    + ' style="' + s_tiannet_day + '">&nbsp;</td>');
...
function tiannetWriteHead() {
    ...
    document.all.tiannetYearHead.textContent = tiannetYear + "年";
    document.all.tiannetMonthHead.innerText= tiannetMonth + "月";
    document.all.tiannetMonthHead.textContent = tiannetMonth + "月";
    if (m_bolShowHour) document.all.tiannetHourHead.innerText= " " + tiannetHour + "时";
    if (m_bolShowHour)  document.all.tiannetHourHead.textContent= " " + tiannetHour + "时";
    if (m_bolShowMinute) document.all.tiannetMinuteHead.innerText = tiannetMinute + "分";
    if (m_bolShowMinute) document.all.tiannetMinuteHead.textContent = tiannetMinute + "分";
    ...
    for (var i = 0; i < 37; i++) {
        ...
        if (tiannetArrDay[i] != "") {
            ...
            if (tiannetArrDay[i] == tiannetDay) {
                $(da).css("backgroundColor","#CCCCCC");
            } else {
                $(da).css("backgroundColor", "#EFEFEF");
            }
            $(da).css("cursor", "pointer");
        } else {
            da.innerHTML = ""; $(da).css("backgroundColor", "");$(da).css("cursor", "default");
        }
    } //end for
    tiannetSetValue(false); //给文本框赋值，但不隐藏本控件
...
//弹出显示日历选择控件，以让用户选择
function tiannetPopCalendar(event) {
    //隐藏下拉框，显示相对应的head
    ...
    //计算left
    if (event.clientX + h > document.body.offsetWidth - 10)
        iX = event.clientX - h - 5;
    else
        iX = event.clientX + 5;
    if (iX < 0)
        iX = 0;
    ...
    if (event.clientY + w > document.body.offsetHeight - 10)
...
//    if (!designPage.isDesign) {
//        src.parentNode.parentNode.parentNode.appendChild(document.getElementById("divTiannetDate"));
    //    }

    iX = getLeft(src);
    iY =getTop(src)+20;
    document.all.divTiannetDate.style.Left = iX;
    document.all.divTiannetDate.style.Top = iY;
    $(document.all.divTiannetDate).css("left", iX);
    $(document.all.divTiannetDate).css("top", iY);
    tiannetShowControl();
    selectObject();
}
//隐藏日历控件(同时显示被强制隐藏的标签)
function tiannetHideControl() {
    $(document.all.divTiannetDate).css("display","none");
    tiannetShowObject();
    arrTiannetHide = new Array(); //将被隐藏的标签对象清空
}
//显示日历控件(同时隐藏会遮挡的标签)
function tiannetShowControl() {
    $(document.all.divTiannetDate).css("display" , "");
...
function tiannetHideObject(strTagName) {
    ...
    x = document.all.divTiannetDate.style.Left;
    y = document.all.divTiannetDate.style.Top;
    h = document.all.divTiannetDate.offsetHeight;
    w = document.all.divTiannetDate.offsetWidth;
    divCount=document.getElementsByTagName(strTagName);
...
//        }
        ...
        if ($(obj).css("display") == "none" || $(obj).css("visibility") == "hidden" || obj.getAttribute("Author") == "tiannet") {
