				...
				var paint = {
					...
					load: function() {
						...
						this.eraserRadius = [5,8,14]; //擦除半径值 
						this.color = ["#000000", "#FF0000", "#80FF00", "#00FFFF"]; //画笔颜色值 
						this.fontWeight = [2, 5, 8, 11, 13];
						this.$ = function(id) {
							return typeof id == "string" ? document.getElementById(id) : id;
						};
//						this.imgurl=this.$('vb_b');
...
//						this.isHuabi = this.$("is_huabi"); //画笔
//						this.boxDel = this.$("box_del"); //
						this.thisCinput = this.$("this_color_input"); //吸色器
						this.revocation = this.$("revocation"); //撤销
					...
					bind: function() {
						var t = this;
						//吸色器
						this.thisCinput.onchange = function() {
							t.cxt.save();
							var the_color = this.value;
							t.cxt.strokeStyle = the_color;
						}
						/*清除画布*/
...
//							var _x = touch.layerX; //鼠标在画布上的x坐标，以画布左上角为起点 
//							var _y = touch.layerY; //鼠标在画布上的y坐标，以画布左上角为起点    
							if(t.isEraser) {
								/* 
...
//							};
						/*橡皮擦*/
						fontIptNum = this.$("is_eraser").children[1].getElementsByTagName("span"); //画笔粗细
							for(var i = 0, l = fontIptNum.length; i < l; i++) {
							fontIptNum[i].onclick = function(e) {
								console.log(this)
								t.isEraser = true;
							};
						}
					...
					changeColor: function() {
						/*为按钮添加事件*/
						var t = this,
							iptNum = this.$("color").getElementsByTagName("a"); //画笔颜色				
						fontIptNum = this.$("font").children[1].getElementsByTagName("span"); //画笔粗细
						for(var i = 0, l = iptNum.length; i < l; i++) {

							iptNum[i].index = i;
							iptNum[i].onclick = function() {
								t.cxt.save();
								console.log(t.color[this.index])
					...
					changeBackground: function(num) {
						...
						var fontIptNum = this.$("font").children[1].getElementsByTagName("span"); //画笔粗细
...
//						this.$("html").src = this.canvas.toDataURL();
//					},
					resetEraser: function(_x, _y, touch,ind) {
						/*使用橡皮擦-提醒*/
						var t = this;
						...
						for(var i=0;i<t.eraserRadius.length;i++){
							t.eraserRadius[i].index = i;
//							console.log(t.eraserRadius[i].eq(index))
							t.cxt.arc(_x, _y, t.eraserRadius[i], 0, Math.PI * 2);
						}						
						t.cxt.strokeStyle = "rgba(250,250,250,0)";
						t.cxt.fill();
						t.cxt.globalCompositeOperation = "source-over"
