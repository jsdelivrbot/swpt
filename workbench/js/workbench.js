...
/*$( document ).ready( function () {
    $.ajaxSetup({
        ...
        beforeSend: function (XMLHttpRequest) {
            ...
            XMLHttpRequest.setRequestHeader( sessionid, 'E9' );
        },
    })
})*/
//权限树渲染方法
...
function getRoleDes() {
    $.ajax( {
        type: "GET",
        // url: root_url + "/htPermissionService/selectPermissionMenuByPosId?posId=" + posId,
        ...
        success: function ( dt ) {
            ...
            window.memberrightsTree = $( "#memberrights" ).ligerTree( {
                ...
                onSelect: function(node) {
                    ...
                    //console.log(node.target.firstElementChild.lastElementChild)
                    //if (node.data.text == "网络硬盘" || ( node.data.id > 217 && node.data.id < 239 && node.data.id != 233 ) ) {
                    if ( node.data.text === $("#memberrights .l-selected span").html() ) {
                        $( '.biaodan' ).fadeIn();
...
//职位权限保存后的成员权限，默认为选中状态
function getRoleDes1() {
    $.ajax( {
        type: "GET",
        // url: root_url + "/htPermissionService/selectPermissionMenuByPosId?posId=" + posId,
        url: root_url + "/htNodeService/selectNodeMenuByPosId?posId=" + posId,
...
function setUserRoleDes() {
    ...
    $.ajax( {
        type: "GET",
        // url: root_url + "/htUserPermissionService/getPermissionIdByPosIdOrUserId?user_id=" + role_user + "&pos_id=" + posId,
...
$( document ).ready( function () {
    $( ".yg" ).on( "click", function () {
        //判断有没有点击职位,如果没有就停止执行
        if ( posId == undefined ) { return false }
        $( '.show_yg' ).fadeIn();
        $( ".show_ys" ).fadeOut();
        maingridReload();
...
$( function () {
    ...
    $( $( ".l-layout-header-inner" )[ 0 ] ).append(
        "<a target='_self' id='website' style='margin-left: 10px;visibility: hidden;'>成员设置</a>" )
...
//console.log($('div[toolbarid=zengjia]').css(""))
    //部门机构树渲染
    $.ajax( {
    ...
    /*$.ajax( {
        ...
        success: function ( data ) {
            ...
            /!*var positionGridLength = positionGrid.getSelectedRows().length;
            console.log(positionGridLength)
            if (positionGridLength === 0){
                alert("请选择角色职位!");
                return false;
            }*!/
            getTreedata( data );
    ...
    function getRoleId(item) {
        ...
        $.ajax( {
            type: "GET",
            url: root_url + 'htNodeService/selectNodeMenuByRoleId?posId=' + item,
            success: function ( data ) {
                console.log(data)
                var msgdata = data.msg;
                dataTree.selectNode( function ( tree ) {
    ...
    // $( "#tab1 li[tabid=tabitem1] a" ).after( "<a id='website'>机构设置</a>" );
    ...
    function romoveScroll() {
        $("html").css("overflow", "hidden")
        var ht = $("html").height();
        $(".indexs").height(ht-54);
        $("#bgc").css("display", "none")
    }
    var companyModalUrl = '../enterprise-old/qiyezhuce.html';
    var workUrl = 'workbenchs.html'
    // var work_gnUrl = 'http://192.168.0.251/editor?pid=1';
    $("#companyModal").click(function(e){
        e.preventDefault();
        romoveScroll();
        $("#maincontents").attr("src", companyModalUrl)
    ...
    $("#gongneng").click(function(e){
        e.preventDefault();
        romoveScroll();
        work_gnUrl="/editor";
        $("#maincontents").attr("src", work_gnUrl)
        $("#gn").css("display", "block")
    })
    $("#liucheng").click(function(e){
        e.preventDefault();
        romoveScroll();
        $("#maincontents").attr("src", newweb_lcUrl);
        $("#gn").css("display", "block")
    })
    $("#system").click(function(e){
        var work_lcUrl = '../system2/index.html';
        e.preventDefault();
        romoveScroll();
        $("#maincontents").attr("src", work_lcUrl)
        $("#gn").css("display", "block")
    })
    $("#enterprise").click(function(e){
        var work_lcUrl = '../enterprise/#';
        e.preventDefault();
        romoveScroll();
        $("#maincontents").attr("src", work_lcUrl)
        $("#gn").css("display", "block")
    ...
    $("#mall").click(function(e){
        var work_lcUrl = '../mall/index.html';
        e.preventDefault();
        romoveScroll();
        $("#maincontents").attr("src", work_lcUrl)
        $("#gn").css("display", "block")
    ...
    //workbench1
    $("#workbench").click(function(e){
        var work_lcUrl = 'workbenchs.html';
        e.preventDefault();
        romoveScroll();
        $("#maincontents").attr("src", work_lcUrl)
        $("#gn").css("display", "block")
...
function treeDataSave() {
    /*var MembersRoleGirdLength = MembersRoleGird.getSelectedRows().length;
    console.log(MembersRoleGirdLength)
    if (MembersRoleGirdLength === 0){
        alert("请选择角色成员用户!");
        return false;
    }*/
...
function treeDataSave2() {
    ...
    /*for ( var i = 0; i < arr.length; i++ ) {
        subdata.nodeIds.push( { "permissionId": arr[ i ].data.id } );
        // myFuc(arr[ i ].data.id);
        // alert(myFuc(arr[ i ].data.id));
    }*/
    var ids = [];
    for ( var i = 0; i < arr.length; i++ ) {
        ids.push( arr[ i ].data.id );
    ...
    //内部递归函数
    /*function myFuc(n) {
        var parent = $('#treedata').tree('getParent', n.target);
    ...
    $.ajax( {
        type: "POST",
        // url: root_url + "/htUserPermissionService/savePermissionAssociateUserByChange",
