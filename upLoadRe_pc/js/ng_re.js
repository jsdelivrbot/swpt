...
r.controller('rectr',["$scope","$rootScope","$state","getData","$stateParams",function($scope,$rootScope,$state,$stateParams,getData){
   ...
   $scope.release = [{"id":1,"title":"全部"},{"id":2,"title":"已发布"},{"id":3,"title":"待审核"},{"id":4,"title":"未通过"}];
    //导航内容
   $scope.navItems = [{'id':1, 'title': '创建资源', 'href':'.conf'},{'id':2,'title': '上传资源',  'href': '.source'},{'id':3, 'title': '我的资源',  'href': '.music'},{'id':4,'title': '资源内容',  'href': '.detail'}];
...
.controller('reUpfileDesctr',["$scope","$rootScope","$routeParams","$compile","getData","$state","$stateParams",function($scope,$rootScope,$routeParams,$compile,getData,$state,$stateParams){
    ...
    $scope.source_type_id = null;
    $scope.source_type_name = null
...
// console.log( $(".uploadcoverName").val()+"."+$("#uploadcover").val().split(".")[1])
     //资源内容跳转 - 编辑跳转 
    ...
    if($stateParams.Cid != null){
        getData.getUrlData("/science/getScienceById?id="+$stateParams.Cid).then(function(d){
            ...
            $scope.selectValue = $scope.sourceName[0].id; //默认显示第一个
            
        })
    }else if($stateParams.ChapId != null){ //我的资源 -- 修改跳转
        getData.getUrlData("/bookChapter/getUnionAllBookChapterById?id="+$stateParams.ChapId).then(function(d){
            ...
            $scope.selectValue = $scope.chapterCon.scienceId;
            $scope.chapterName = $scope.chapterCon.bookChapterName;
            $scope.fileVideoName = $scope.chapterCon.videoList[0].videoName;
            $scope.fileAudioName = $scope.chapterCon.musicList[0].musicName;
            //$("#selName").attr("disabled","disabled");
            $scope.sourceName.push({"id":$scope.chapterCon.scienceId,"bookName":$scope.chapterCon.scienceList[0].bookName});
    ...
    $scope.sw_teaching = function(){
        $scope.cmp = angular.copy($scope.cmp);//复制模板
        $("#p_sw_material").show();
    }
    ...
    $scope.clearNoNum = function(obj,attr){
        ...
        obj.file_price = obj.file_price.replace(/[^\d.]/g, "");  //先把非数字的都替换掉，除了数字和"."
      
        obj.file_price = obj.file_price.replace(/^\./g, "");//必须保证第一个为数字而不是"."
     
        obj.file_price = obj.file_price.replace(/\.{2,}/g, "."); //保证只有出现一个.而没有多个"."
      
        obj.file_price = obj.file_price.replace(".", "$#$").replace(/\./g, "").replace("$#$", ".");//保证"."只出现一次，而不能出现两次以上
      
         obj.file_price = obj.file_price.replace(/([0-9]+\.[0-9]{2})[0-9]*/, "$1"); //保证"."只后面只能出现两位有效数字
    }
    ...
    $scope.nextFile = function(){
        ...
        var formdata = new FormData(form);
           //判断是创建还是编辑页面过来
       if( $stateParams.id != null){ // 从编辑页面过来
            ...
            $scope.sourceId = $scope.cmp.source_type_id;
            $scope.bookId = $scope.cmp.book_type_id;
            $scope.freeId = $scope.cmp.free_type_id;
            $scope.recId  = $scope.cmp.rec_object_id;
   ...
   //上传文件类型前端显示 
   $scope.upFile = function(mb,file,file_length,reader,upfile_type,up,uname,fname,fileName){
   ...
   $scope.fileTest = function(file,file_max,type){
    ...
    $scope.fileSelected=function(type){
        ...
        var uname = $(".re_uf_bldes ."+type+"Name");
         $scope.fileName = file[0].name.split(".")[0];
        $("."+type+"content").show();
        ...
        if(file[0].name!=''){
            if(type=='uploadvideo'){
                ...
                if($scope.des.video.upfile_video_ext.indexOf(fileType) !== -1){ 
                    $scope.upFile(mb,file,file_length,reader,upfile_type,up,uname,fname,$scope.fileName);
                }else{
        ...
        $scope.now = year+'-'+p(month)+"-"+p(date)+" "+p(h)+':'+p(m)+":"+p(s);
            ...
            $.ajax({
                ...
                success: function (d) {
                        ...
                        //成功后弹出窗口
                    if($stateParams.ChapId != null){
                            swal("恭喜您!修改成功","", "success");
                    }else{
                            swal("恭喜您!上传成功", "", "success"); 
                    }
 ...
 //默认获取分页总数
    ...
    $scope.uploadVideo = function(id,ev){
        ...
        swal({ 
            ...
            text: "<video src='"+$scope.imgSrc+$scope.dataUrl+"' style='width:400px;height:400px ;object-fit:fill' controls='controls'></video>", 
            html: true 
          });
    }
    //双击我的资源 - 查看资源 
    $scope.uploadMusic = function(id,ev){
        ...
        swal({ 
            title: ""+$scope.dataName+"",
            text: "<audio src='"+$scope.imgSrc+$scope.dataUrl+"' style='width:400px;height:50px' controls='controls'></audio>", 
            html: true 
          });
    }
...
}]).controller('detail',["$scope","$rootScope","$state",'getData','$stateParams',function($scope,$rootScope,$state,getData,$stateParams){
    $scope.examine = 1; //默认审核 -- 显示全部
    $scope.stateList = [];  //审核状态       
    $scope.gId = null ; //资源内容  id
    $scope.sourceConent  = null;  //获取资源内容 
    $scope.examineId = 1 ;    //审核 显示 id
    $scope.examineId = null; //审核id
    $scope.imgSrc = source_exam; //图片路径
    $scope.release = [{"id":1,"title":"全部"},{"id":2,"title":"已发布"},{"id":3,"title":"待审核"},{"id":4,"title":"未通过"}];
     //分页默认参数  
     $scope.paginationConf = {
    	currentPage:1,
   ...
   $scope.getState = function(data){
    ...
    $scope.examineType = function(id,name){
        ...
        $scope.sourceDetail = function(ev){
           ...
           $state.go("main.bookDetail",{detailId:$scope.detailId,examineId:$scope.examineId,bookExamId:$scope.bookExamId});
       }
    //点击继续上传   
    $scope.gid = 2 
    $scope.ContinueUpload = function(e){
        e.stopPropagation();
      $scope.bookExamId =angular.element(e.target).parent().parent().parent().parent().attr("data-exam");
      $scope.gId  =      angular.element(e.target).parent().parent().parent().parent().attr("id");
      //判断审核通过的章节才能继续上传视频
        if($scope.examineId == 2 || $scope.examineId == 3 || $scope.bookExamId == 2 || $scope.bookExamId == 3 ){
            getData.getUrlData("/science/getScienceById?id="+$scope.gId).then(function(d){
            $scope.dataUpload = d.data.msg;
            $scope.$emit('tab',2);
            $state.go('main.source',{
                "Cid":$scope.gId
                });
            ...
            layer.alert("sorry~请通过审核再上传");
        }
    }

    //章节管理
    $scope.chapterManage = function(ev){
        ev.stopPropagation();
        $("body").append('<div class="modal-backdrop fade in"></div>');
        $("#myModal").show();
        $("#myModal").css("opacity",1);

        $scope.chapterId = $(ev.target).parent().parent().parent().parent().attr("id");
        getData.getUrlData("/science/getMultiTableScienceById?id="+$scope.chapterId).then(function(d){
              if(d.data.msg == null){
                 return false;
              } else{
                $scope.chapterData = d.data.msg.chapterList; 
              }
    ...
    //编辑资源
    $scope.editorSource = function(e){
        e.stopPropagation();
        $scope.gId  = angular.element(e.target).parent().parent().parent().parent().attr("id"); 
         console.log($scope.gId)
        getData.getUrlData("/science/getScienceById?id="+$scope.gId).then(function(d){
           $scope.data = d.data.msg;
           $scope.$emit('tab',1);  //传 1 父控制器中的 导航栏改变 
           $state.go('main.editor',{id:$scope.gId});
           localStorage.clear(); //清除页面中的缓存，防止数据重复加载
        }) 
    ...
    $scope.delSource = function(e){
        e.stopPropagation(); 
        $scope.ele = angular.element(e.target).parent().parent().parent().parent();
      //获取点击资源的id
      $scope.gId  = $scope.ele.attr("id"); 
      //删除资源内容
        swal({
            title: "您确定要删除吗？",
            text: "您确定要删除这条数据？",
