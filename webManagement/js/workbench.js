...
var getSearchInfoRole = root_url + "/htRoleService/getSearchInfoRoleBySite"
//根据选择的部门id,显示权限
// var getPowerMenu = root_url + "/htPositionNodeService/getNodeIdByPosId?posId="
var getPowerMenu = root_url + "/htPositionNodeWqglService/getNodeWqglIdByPosId?posId="
...
var getAddRoleChildNode = root_url + "/htRoleService/getAddRoleChildNodeBySite?"
//添加兄弟节点 地址
// var getAddBrotherNode = root_url + "/htRoleService/getAddRoleNode?"
var getAddBrotherNode = root_url + "/htRoleService/getAddRoleNodeBySite?"
//修改节点  地址
...
$.ajax( {
    ...
    url: root_url + "/htNodeWqglService/selectNodeWqglMenuByPosId?posId=" + posId,
    success: function ( dt ) {
        var mydata = [];
        if ( dt.status == 0 ) {
            mydata = dt.msg;
...
$.ajax( {
    ...
    url: root_url + "/htUserNodeWqglService/getNodeWqglIdByPosIdOrUserId?user_id=" + role_user + "&pos_id=" + posId,
    success: function ( data ) {
        var msgdata = data.msg;
        //console.log(msgdata)
        memberrightsTree.selectNode( function ( tree ) {
...
$.ajax( {
    type: "GET",
    // url: root_url + "/htNodeService/selectNodeMenuBySite",
    url: root_url + "/htNodeWqglService/getNodeWqglMenu",
    success: function ( data ) {
        //角色权限树方法调用
        getTreedata( data );
...
function addChildN( item, i ) {
    ...
    $.ajax( {
        ...
        type: "GET",
        success: function ( data ) {
            console.log(getAddRoleChildNode + "rolename=" + rolename + "&description=" + remark + "&role_id=" +_actionCustomerID)
            if ( data.status == 1 ) {
                $.ajax( {
                    type: "GET",
                    url: getSearchInfoRole,
...
// $( "#tab1 li[tabid=tabitem1] a" ).after( "<a id='website'>机构设置</a><a href='http://192.168.0.193?pid=1'>功能设置</a><a href=''>流程设置</a>" );
...
// var gnUrl = 'http://192.168.0.251/editor?workbench';
...
$("#websites").click(function(e){
    e.preventDefault();
    romoveScroll();
    $("#maincontents").attr("src", web_gnUrl)
    $("#gn").css("display", "block")
})
...
/*if ( subdata.nodeIds.length > 0 ) {
    $( '.l-checkbox-incomplete' ).closest( 'li' ).each( function ( o ) {
        var tmpid = $( this ).attr( 'id' );
        if ( tmpid > 0 && $.inArray( tmpid, ids ) == -1 ) {
            subdata.nodeIds.push( { "permissionId": tmpid } );
        }
    } )
}*/
$.ajax( {
    ...
    url: root_url + "/htPositionNodeWqglService/getUpdateOrDeleteNodeWqglMenu",
...
$.ajax( {
    ...
    url: root_url + "/htUserNodeWqglService/saveNodeWqglAssociateUserByChange",
    data: JSON.stringify( subdata ),
    contentType: "application/json",
    async: false,
    success: function ( data ) {
